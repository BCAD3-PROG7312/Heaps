...
namespace Heaps.PQueues
{
    class PriorityQueue
    {
        ...
        private int size = 0;
        public NGCCNode [] items = new NGCCNode[capacity]; // we now use an array of nodes instead of ints
        ...
        private NGCCNode leftChild (int index)
        {
            return items[getleftChildIndex(index)];
        }

        private NGCCNode rightChild (int index)
        {
            return items[getRightChildIndex(index)];
        }

        private NGCCNode parent (int index)
        {
        ...
        {
            NGCCNode temp = items[indexOne];// store index one in a temp value 
        ...
        public NGCCNode Dequeue() // luck pop but for heaps 
        {
            if (size == 0)
            {
                throw new NullReferenceException();
        ...
        public void Enqueue (int item, int priority)
        {
            ...
            items[size] = new NGCCNode(item, priority);
            //grow array 
            size++;
            heapifyUp();
        }
